    ...
    module Categories
      module Entities
        class IndexForShopperEntity < API::BaseEntity
          expose :non_empty_categories, using: API::V2::Categories::Entities::ShowEntity, as: :categories, documentation: {type: 'show_category', is_array: true }
          ...
          def non_empty_categories
            ...
            object[:categories].each do |cat|
              if retailer_id.present?
                counter = Brand.joins(:retailers, :categories)
                ...
                counter = Brand.connection.select_all("SELECT count(DISTINCT b.id) AS brand_count FROM brands as b
                          ...
                          INNER JOIN categories AS c ON sc.parent_id = c.id
                          ...
                          c.id  = %{parent_id};" % {parent_id: cat.id, location_id: location_id})
              end
