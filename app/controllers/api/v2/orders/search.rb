        ...
        resource :orders do
          ...
          get '/search' do
            ...
            order = order.includes({ order_positions_views: [:product, :order, { order_subs_view: [
                                     :shop_promotion, { substituting_product: %i[brand categories subcategories retailer_shops] }] }] })
            order = order.includes(:collector_detail, :pickup_loc, vehicle_detail: %i[color vehicle_model])
            order = order.includes({ analytics: :event })
            result = order.first
            if result
              present result, with: API::V2::Orders::Entities::ShowOrderEntity, retailer: true
            else
