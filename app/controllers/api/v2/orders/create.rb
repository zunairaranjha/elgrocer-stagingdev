      ...
      class Create < Grape::API
        ...
        helpers do
          ...
          def order_params
            ...
            _params[:status_id] = payment_type.id == 3 ? -1 : 0
            _params
          end

          def create_order
            order = Order.find_by(retailer_id: retailer.id, status_id: -1, shopper_id: current_shopper.id)
            if order
              clear(order.id)
              order.attributes = order_params.compact
              order.created_at = Time.now
            else
              order = Order.new(order_params.compact)
            end
