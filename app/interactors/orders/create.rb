class Orders::Create < Orders::Base
  ...
  string :app_version, default: nil
  ...
  def execute
    ...
    Order.transaction do
      ...
      if promo_realization.present? && promotion_code.can_be_used?(shopper_id, retailer_id)
        ...
        promo_realization.retailer_id = order.retailer_id unless payment_type_id == 3 and card_id.to_i == 0
        promo_realization.save
      else
        PromotionCodeRealization.where(order_id: order.id).update_all(retailer_id: nil, order_id: nil)
      end
      if payment_type_id == 3 and card_id.to_i == 0
        order.status_id = -1
      else
        order.status_id = 0
        retailer.new_order_notify(order.id)
      end
      order.total_value = get_overall_value
      order.save
      order
    end
  end
  ...
  def promo_realization
    @promo_realization ||= PromotionCodeRealization.find_by(id: promotion_code_realization_id)
  end
  ...
  def order_params
    params = {
      ...
      shopper_phone_number: shopper_address.phone_number || shopper.phone_number,
      shopper_name: shopper_address.shopper_name || shopper.name,
      ...
      app_version: app_version || 'WEB',
      retailer_service_id: 1
    }
    
    # params[:delivery_slot_id] = delivery_slot_id if delivery_slot.present?
    params[:estimated_delivery_at] = delivery_slot.present? ? cal_estd_dlvry_at : Time.now + 1.hour
  ...
  def create_order!
    order = shopper.orders.find_by(retailer_id: retailer.id, status_id: -1)
    if order
      clear(order.id)
      order.update(order_params.compact)
      order.created_at = Time.now
    else
      order = Order.create!(order_params.compact)
    end
  ...
  def get_overall_value
    if @overall_value.to_i < 1
      ...
      # db_categories = Category.unscoped.include(category_id: get_category_ids).where(current_tags: Category.tags["pg_18"])
  ...
  def promocode_invalid
    errors.add(:promocode_is_valid, 'Invalid promotion code') if promotion_code_realization_id.present? && (!promotion_code.can_be_used?(shopper_id, retailer_id) || !accepted_promocode)
  end
  ...
  def promocode_invalid_brands
    if promotion_code_realization_id.present? && promotion_code
  ...
  def delivery_slot_products_limit
    # errors.add(:delivery_slot_products_limit, "Sorry! Someone else just snatched that delivery slot. Please select another.") if delivery_slot.present? && delivery_slot.products_limit > 0 && ((orders_products_amount + products_amount) > (delivery_slot.products_limit + delivery_slot.products_limit_margin)) and ((orders_products_amount/(delivery_slot.products_limit * 1.0)) > 0.7)
  ...
  def retailer_delivery_type_invalid
    errors.add(:retailer_delivery_type_invalid, "Oops! This time slot is taken now, please select another.") if retailer_delivery_zone and ((retailer_delivery_zone.instant? and delivery_slot.present?) or (retailer_delivery_zone.schedule? and delivery_slot.blank?))
  end
end
